...
const EventCard: React.FC<EventCardProps> = ({
  ...
  const router = useRouter();
   ...
   const [deletePopup, setDeletePopup] = useState(false)
    const { deleteEvent, isDeletingEvent } = mutations()


     const handleDelete = async (eventId: string) => {
  try {
    await deleteEvent(eventId);       // 👈 pass eventId here
  } catch (error) {
      ...
      <div className="grid place-items-center fixed w-full top-0 bottom-0 right-0 left-0 z-[100]">
        <div className="bg-[#131313] border border-[#ffffff65] h-fit w-fit px-8 py-9  rounded-md space-y-7 max-sm:w-[90%] ">
          ...
          <div className="grid grid-cols-2 gap-5 w-full">
            <Button loading={isDeletingEvent} loaderClass="mt-[0.16em] ml-[-0.005em]" className="font-medium px-3 py-2 rounded-md h-fit bg-primary hover:scale-[1.04] transition" onClick={() => handleDelete(eventId?? "")} >Yes</Button>
