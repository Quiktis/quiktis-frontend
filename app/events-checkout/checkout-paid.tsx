...
export default function CheckoutPaid() {
  ...
  return (
    <div className="bg-[#131517] min-h-screen text-gray-300 font-sans p-4 sm:p-6 md:p-8">
      ...
      {showTicketPopup && (
        <div
          className="hide-scrollbar fixed inset-0 w-screen h-screen flex items-center justify-center p-5 overflow-auto z-[9999] bg-[#131517F7]"
          onClick={() => setShowTicketPopup(false)}
        >
          <div
            className="hide-scrollbar bg-transparent w-full max-w-[1250px] flex flex-col items-start pt-20 pb-10 max-h-[90vh] overflow-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-6 -mt-[72px]">
              <button
                onClick={() => setShowTicketPopup(false)}
                className="w-[124px] h-[42px] bg-transparent border border-[#373737] rounded-[5px] flex-shrink-0 min-w-[124px] whitespace-nowrap font-medium text-xl leading-[100%] text-white cursor-pointer flex items-center justify-center gap-2 font-inter"
              >
                <svg
                  width="4.46"
                  height="8.11"
